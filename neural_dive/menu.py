"""
Menu system for Neural Dive content selection.

Provides interactive terminal UI for selecting content sets before game starts.
"""
...
# Note: The show_difficulty_menu function has been removed.
# The game now always uses NORMAL difficulty settings.
# This keeps the code simple until proper difficulty-based question filtering is implemented.
    ...
    with term.cbreak(), term.hidden_cursor():
        while True:
            ...
            if key.name == "KEY_UP":
                ...
                return str(content_metadata[selected_index][0])
            elif key.isdigit():
                idx = int(key) - 1
                if 0 <= idx < len(content_metadata):
                    return str(content_metadata[idx][0])
            elif key.lower() == "q":
                sys.exit(0)
